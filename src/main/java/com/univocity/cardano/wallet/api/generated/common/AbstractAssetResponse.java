...
public abstract class AbstractAssetResponse {
	...
	public void setFingerprint(String fingerprint){
		if (fingerprint == null) {
			throw new IllegalArgumentException("Value of fingerprint cannot be null");
		}

		if (fingerprint.codePointCount(0, fingerprint.length()) < 44) {
			throw new IllegalArgumentException("Length of fingerprint must have at least 44 characters, got '" + fingerprint.codePointCount(0, fingerprint.length()) + "'");
		}

		if (fingerprint.codePointCount(0, fingerprint.length()) > 44) {
			throw new IllegalArgumentException("Length of fingerprint cannot exceed 44 characters, got '" + fingerprint.codePointCount(0, fingerprint.length()) + "'");
		}
	...
	/**
	 * Defines the metadata error (optional).
	 * 
	 * If there was an error fetching metadata from the server,
	 * this will be set to one of `fetch` or `parse`, to indicate
	 * the kind of error.
	 ...
	 * - Accepted values: {@code [fetch, parse]}.
	 * 
	 * @param metadataError the metadata error
	 */
